...
# App init
# ----------------------------
app = FastAPI(title="Honey Adulteration API", version="0.3.1")

# CORS (for Flutter / mobile during development)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Paths
# ----------------------------
PROJECT_ROOT = os.path.dirname(os.path.dirname(__file__))  # honey_project/
...
# Create optional UI/data dirs so StaticFiles mounts do not crash even if you only use the JSON API.
...
def build_cnn_backbone():
    ...
    model.classifier = torch.nn.Identity()  # output: 1280 features
    model.eval().to(DEVICE)

    preprocess = weights.transforms()  # ✅ correct for your torchvision version
...
def extract_img_feat(img_path: str) -> np.ndarray:
    ...
    return feat.astype(np.float32)  # (1280,)
...
def predict_fusion(img_path: str, density: float, ph: float, flow_time: float):
    ...
    proba = RF_MODEL.predict_proba(X)[0]  # [p0, p1]
...
async def predict_api(
    ...
    Returns:
      { "label": "PURE" | "ADULTERATED", "confidence": 0..1, "p_adulterated": 0..1 }
    """
    # Basic extension validation (optional)
...
def ui_suggestions(q: str = ""):
    t = normalize_q(q)
    if not t:
        return {"suggestions": SUGGESTIONS[:8]}
    sug = [s for s in SUGGESTIONS if t in s.lower()]
    if not sug:
        sug = [s for s in SUGGESTIONS if s.lower().startswith(t)]
    return {"suggestions": sug[:8]}
...
def ui_history(
    ...
    base = apply_filters(base, q, result_type, min_conf)

    total = base.count()
    pages = max(1, (total + per_page - 1) // per_page)

    page = min(page, pages)
    rows = (base.order_by(Prediction.id.desc())
...
@app.get("/ui/details/{pred_id}", response_class=HTMLResponse)
def ui_details(pred_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "details.html",
        {"request": request, "row": row, "image_url": image_url, "score_pct": None}
    )
...
def admin_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    pure = db.query(Prediction).filter(Prediction.result.ilike("%pure%")).count()
    adulter = db.query(Prediction).filter(Prediction.result.ilike("%adulter%")).count()
    uncertain = total - pure - adulter if total else 0

    avg_conf = 0.0
    if total:
        avg_conf = sum(r[0] for r in db.query(Prediction.confidence).all()) / total

    last10 = db.query(Prediction).order_by(Prediction.id.desc()).limit(10).all()
    ...
    stats = {
        "total": total,
        "pure": pure, "pure_pct": pct(pure),
        "adulter": adulter, "adulter_pct": pct(adulter),
        "uncertain": uncertain, "uncertain_pct": pct(uncertain),
        "avg_conf": round(avg_conf, 3),
    }

    return templates.TemplateResponse(
        "admin_dashboard.html",
        {"request": request, "stats": stats, "last10": last10}
    )
...
@app.get("/admin/details/{pred_id}", response_class=HTMLResponse)
def admin_details(pred_id: int, request: Request, db: Session = Depends(get_db)):
    require_admin(request)
...
async def upload_csv(file: UploadFile = File(...)):
    try:
        ...
        # ✅ الحالة 1: القيم تحت بعضها (عمود واحد)
        if df.shape[1] == 1:
            row = df.iloc[:, 0].dropna().tolist()

        # ✅ الحالة 2: القيم في سطر واحد
        else:
            row = df.iloc[0].dropna().tolist()

        print("✅ Parsed values:", len(row))

        if len(row) != 128:
            raise HTTPException(
                status_code=400,
                detail=f"CSV must contain 128 values, got {len(row)}"
            )
